<?php
	...
	for($i = 1; $i <= $max; $i++){
		...
		while($data = mysqli_fetch_array($result_replays)){
			...
			$replaysPerWeek = $replaysPerWeek + ($db_replays - 1);			//fetch data and add (-1 because replays are listed as 2 Plays but it is only 1 is replay)
		}
		...
		$avgReplaysPerDay = round($avgReplaysPerDay, 2);					//round to 2 decimal place
		$avgReplaysPerDay = number_format($avgReplaysPerDay, 2, '.', '');			//number_format(VALUE, decimal place value)
